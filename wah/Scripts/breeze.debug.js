    ...
    ajaxImpl.ajax({
      ...
      success: function (httpResponse) {
        ...
        try {
          ...
          return handleHttpError(deferred, httpResponse, "1 Metadata query failed for: " + url + ". " + errMsg);
        }
      ...
      error: function (httpResponse) {
        handleHttpError(deferred, httpResponse, "2 Metadata query failed for: " + url);
      }
    });
...
;(function (factory) {
    ...
    OData.read({
        ...
        function (data) {
          ...
          if (!data || !data.dataServices) {
            var error = new Error("3 Metadata query failed for: " + url);
          ...
          if (!metadataStore.hasMetadataFor(serviceName)) {
            try {
              ...
              return deferred.reject(new Error("4 Metadata query failed for " + url + "; Unable to process returned metadata: " + e.message));
            }
          ...
          err.message = "6 Metadata query failed for: " + url + "; " + (err.message || "");
          return deferred.reject(err);
        },
        ...
        br.__changeResponses.forEach(function (cr) {
          ...
          if ((!statusCode) || statusCode >= 400) {
            ...
            if (response && response.headers) {
               var contentId = response.headers["Content-ID"];
               if (contentId && contentKeys) {
                   entityData = contentKeys[contentId];       
               }
            }
           
            deferred.reject(createError(cr, url, entityData));
            return;
          }
  ...
  function createError(error, url, entity) {
